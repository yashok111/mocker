...
func TestAssets_putRefusals(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"generic XML at the chain", rawPut(t, id, "a.xml", "application/xml", []byte("<sample/>"), cookie, csrf), http.StatusUnsupportedMediaType, "unsupported_media_type"},
		{"text XML at the chain", rawPut(t, id, "b.xml", "TEXT/XML; charset=utf-8", []byte("<sample/>"), cookie, csrf), http.StatusUnsupportedMediaType, "unsupported_media_type"},
		{"XML suffix at the chain", rawPut(t, id, "c.xml", "application/atom+xml", []byte("<sample/>"), cookie, csrf), http.StatusUnsupportedMediaType, "unsupported_media_type"},
