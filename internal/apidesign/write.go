...
func (r *Repo) Create(ctx context.Context, in CreateInput) (*Detail, error) {
	...
	prepared, err := r.prepare(in.Document)
	if err != nil {
		return nil, err
	}
	var result *Detail
...
func (r *Repo) Save(ctx context.Context, id int64, in SaveInput) (*Detail, error) {
	...
	err = r.db.Write(ctx, func(tx *sql.Tx) error {
		...
		if in.ChangeSetID != nil {
			...
			if set.Status != "open" {
				return invalidField("/changeSetId", "Набор изменений закрыт")
			}
		}
		...
		if old.Hash == p.hash && !in.ForceRevision {
			result, err = detailTx(ctx, tx, id)
			return err
		}
